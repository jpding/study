...
function _sendMessage() {
    ...
    for (var i = 0, n = rows.length; i < n; i++) {
        _sendMessageByRow(rows[i]);
    }
}

function _sendMessageByRow(row) {
    if (!row) return;
    var orgs = row[12];
...
var SEND_MESSAGE_ACTION = sz.sys.ctx('/meta/xtjg/collections/cj/F_QTXX/resources/sendmsg');

function _querySendMessage(args) {
    $.post(SEND_MESSAGE_ACTION, args, function(d) {});
}

$.get(SEND_MESSAGE_ACTION, { 'method': 'list' }, function(d) {
    if (!d) return;
    var sys = sz.sys;
    var utils = sz.utils;
    var Message = sys.namespace('sz.portal.Message');

    Message.reply = function(env) {
        var rs = $(this);
        $.post(SEND_MESSAGE_ACTION, {
            'method': 'reply',
    ...
    Message.markAsRead = function(env) {
        var rs = $(this);
        $.post(SEND_MESSAGE_ACTION, {
            'method': 'markAsRead',
            'sessionid': rs.data('sessionid')
    ...
    Message.show = function(args) {
        ...
        if (!dlg) {
            ...
            if (window['szrequire']) szrequire(['sz.commons.dialog'], initDlg);
            else initDlg();
            return;
        }
        dlg.show({
