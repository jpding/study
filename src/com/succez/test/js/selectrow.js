...
(function($) {
	...
	widlg.showWIFormDialog = function(residOrPath, formdata, width, height) {
		if (!window.formDlg) {
...
sz.commons.SelectableTable.createSelectable = function(tableid) {
	var selectTable = $rpt.getCurrentBodyDom().find(tableid);
	return sz.commons.SelectableTable.build(selectTable);
}
...
$rpt.ondisplay = function() {
	...
	window.selectableTable = sz.commons.SelectableTable.build(selectTable);
}

var drill = $(".sz-bi-prst-drillpath").children("li:first").find("a").attr("href");
